...
import { TextField, TextFieldInput } from "~/lib/components/ui/text-field";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
...
const SidebarProvider: Component<SidebarProviderProps> = (rawProps) => {
  ...
  const setOpen = (value: boolean | ((value: boolean) => boolean)) => {
    ...
    _setOpen(value);
    const cookieValue = open() ? "true" : "false";

    // This sets the cookie to keep the sidebar state.
    document.cookie = `${SIDEBAR_COOKIE_NAME}=${cookieValue}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE.toString()}`;
  };
